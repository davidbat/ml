...
if mode != "test":
	...
	W_I_H = initializeW(hidden_nodes, feature_num + bias) # 3 by 8 i.e. 3 arrays with 8 vals each
	W_H_O = initializeW(feature_num, hidden_nodes + bias) # 8 by 3 i.e. 8 arrays with 3 vals each
else:
	#BIAS_H = [-0.4, 0.2]
	#BIAS_O = [0.1]
	BIAS_H = [0.0, 0.0]
	BIAS_O = [0.0]
	W_I_H = [[0.2, 0.4, -0.5, -0.4], [-0.3, 0.1, 0.2, 0.1]] # 2 by 3
	W_H_O = [[-0.3, -0.2, 0.1]]


def check_diff(trop, trip, W_I_H, W_H_O, BIAS_H, BIAS_O):
	for t in range(0, len(trip)):
		Hidden_Out = calculate_sig(trip[t] +bias_lst, W_I_H, BIAS_H)
		Output_Out = calculate_sig(Hidden_Out +bias_lst, W_H_O, BIAS_O)
...
#print W_I_H
#print W_H_O
delta_W_I_H = [[0.0 for x in range(feature_num+bias)] for y in range(hidden_nodes)]
delta_W_H_O = [[0.0 for x in range(hidden_nodes+bias)] for y in range(feature_num)]

cnt = 0
cond = True
prev_mse = 999999999
while cond:
	learning_rate = random.uniform(0,0.3)
	mse = 0
	prev_W_I_H = list(W_I_H)
	prev_W_H_O = list(W_H_O)
	for t in range(0, len(trip)):
		print trip[t] + bias_lst
		print W_I_H
		print BIAS_H
		Hidden_Out = calculate_sig(trip[t] + bias_lst, W_I_H, BIAS_H) 
		print "HO - ", Hidden_Out
		...
		update_W(W_H_O, Net_Err, Hidden_Out + bias_lst, delta_W_H_O)
		update_W(W_I_H, Hid_Err, trip[t] + bias_lst, delta_W_I_H)
	delta_W_I_H = my_sub(W_I_H, prev_W_I_H)
	delta_W_H_O = my_sub(W_H_O, prev_W_H_O)

	cnt += 1
	#print itr , "(((((((((((((((((((((((((((((
	cond = (not check_diff(trop, trip, W_I_H, W_H_O, BIAS_H, BIAS_O)) and (cnt <= itr)
...
print "Hidden Vals\t\t\t\t\tRounded Hidden Values"
temp_arr = []
for t in range(0, len(trip)):
#	print trip[t]
	Hidden_Out = calculate_sig(trip[t] + bias_lst, W_I_H, BIAS_H) 
